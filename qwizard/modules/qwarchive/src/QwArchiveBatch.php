 ...
 */
class QwArchiveBatch {

  /**
   * Batch callback to archive user data.
   */
  public static function archiveUserData($uid, $data_types, &$context) {
    $account = \Drupal::entityTypeManager()->getStorage('user')->load($uid);
    $context['message'] = t('Archiving data for %name',
      ['%name' => $account->getDisplayName()]
    );
    $data_type_callbacks = \Drupal::service('qwarchive.manager')->getDataTypeCallbacks();
    ...
    foreach ($data_types as $type) {
      ...
      try {
        $process_status = \Drupal::service('qwarchive.manager')->$callback($account);
        ...
        \Drupal::logger('qwarchive')->error("Data Archival for user $uid for $type failed with error: $message");
   ...
   */
  public static function archiveUserDataFinished($success, $results, array $operations) {
      ...
      if (!empty($results['data_process'])) {
        ...
        foreach ($results['data_process'] as $data_process) {
          ...
          if (!empty($failed_items)) {
            $messenger->addError('Archival failed for %name: %types.', [
              '%name' => $account->getDisplayName(),
              '%types' => implode(', ', $failed_items),
            ]);
            // The archival process is not completed.
            \Drupal::service('qwarchive.record_manager')->markArchivalAsFailed($uid, 'Failed: ' . implode(', ', $failed_items));
          }
          else {
            // All items are successful.
            \Drupal::service('qwarchive.record_manager')->markArchivalAsCompleted($uid, 'Successful via batch.');
          }
        }
      }
    }
